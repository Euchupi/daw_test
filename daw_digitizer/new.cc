...
int main()
{
	...
	// Multi board inforamtion 
	/*
	ret |= CAEN_DGTZ_ReadRegister(Nhandle, 0x811C, &regval);
	ret |= CAEN_DGTZ_WriteRegister(Nhandle, 0x811C, regval | 0x4d0138);
	// acquisition mode is sw-controlled for the first board, LVDS-controlled for the others
	ret |= CAEN_DGTZ_ReadRegister(Nhandle, 0x8100, &regval);
	ret |= CAEN_DGTZ_WriteRegister(Nhandle, 0x8100, regval | (board == 0 ? 0x00000100 : 0x00000107));
	// register 0x816C: reduces the threshold at which the BUSY is raised at 2^buffer organization-10 events
	ret |= CAEN_DGTZ_ReadRegister(Nhandle, 0x800C, &regval);
	ret |= CAEN_DGTZ_WriteRegister(Nhandle, 0x816C, (uint32_t)(pow(2., regval) - 10));
	// register 0x8170: timestamp offset
	ret |= CAEN_DGTZ_WriteRegister(Nhandle, 0x8170, 3 * (ConfigVar->Nhandle - board - 1) + (board == 0 ? -1 : 0));
	// register 0x81A0: select the lowest two quartet as "nBUSY/nVETO" type. BEWARE: set ALL the quartet bits to 2
	ret |= CAEN_DGTZ_ReadRegister(Nhandle, 0x81A0, &regval);
	ret |= CAEN_DGTZ_WriteRegister(Nhandle, 0x81A0, regval | 0x00002222);
	*/ 
	...
	for (i=0; i<16;i++)
	{
		...
		{
			ret |= CAEN_DGTZ_ReadRegister(Nhandle, 0x1080 | (channel << 8), &regvalue);
			ret |= CAEN_DGTZ_WriteRegister(Nhandle, 0x1080 | (channel << 8), regvalue | (uint32_t)(0x100));
		}
		else
		{
			ret |= CAEN_DGTZ_ReadRegister(Nhandle, 0x1080 | (channel << 8), &regvalue);
	...
	FILE ***WaveFile;
	WaveFile = (FILE***)calloc(1, sizeof(FILE**));
	// The ** must allocate 
